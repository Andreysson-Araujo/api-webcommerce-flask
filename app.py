...
app = Flask(__name__)
app.config['SECRET_KEY'] = "dinossauro_123"
...
def add_to_cart(product_id):
  ...
  if user and product:
    ...
    db.session.commit()
    return jsonify({'message': ' Item added to the car successfully'})
...
def remove_from_cart(product_id):
  ...
  if cart_item:
    ...
    db.session.commit()
    return jsonify({'message': 'Item removed from the car successfully'})
